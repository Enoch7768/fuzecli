...
func New(name,apiKey,baseURL,env string,maxInput,maxOutput,jsonOutput int) provider.Provider{if apiKey==""{apiKey=os.Getenv(env)};return &Provider{name:name,apiKey:apiKey,baseURL:strings.TrimRight(baseURL,"/"),env:env,maxInput:maxInput,maxOutput:maxOutput,jsonOutput:jsonOutput,client:&http.Client{Timeout:120*time.Second}}}
