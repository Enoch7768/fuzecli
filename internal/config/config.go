...
func Load() (Config, error) {
	...
	c := Default(); if err := parseYAML(string(b), &c); err != nil { return Config{}, fmt.Errorf("parse config: %w", err) }; syncProviderConfiguration(c); return c, nil
}
...
func Save(c Config) error {
	...
	data := renderYAML(c); tempPath := Path() + ".tmp"
	...
	syncProviderConfiguration(c); return nil
}

func Set(key, value string) error {
	c, err := Load(); if err != nil { return err }; parts := strings.Split(key, ".")
	switch {
	case key == "default_provider": if value == "" { return fmt.Errorf("default_provider cannot be empty") }; c.DefaultProvider = value
	case key == "fallback_order": order := parseList(value); if len(order) == 0 { return fmt.Errorf("fallback_order cannot be empty") }; c.FallbackOrder = order
	case len(parts) == 3 && parts[0] == "providers":
		name, field := parts[1], parts[2]; p := c.Providers[name]
		switch field { case "api_key": p.APIKey = value; case "default_model": p.DefaultModel = value; case "base_url": p.BaseURL = value; default: return fmt.Errorf("unsupported config field %q", field) }; c.Providers[name] = p
	case key == "verification.self_correction_attempts": n, err := strconv.Atoi(value); if err != nil || n < 0 { return fmt.Errorf("invalid self-correction attempt count %q", value) }; c.Verification.SelfCorrectionAttempts = n
...
func renderYAML(c Config) string {
	var b strings.Builder; fmt.Fprintf(&b, "default_provider: %s\n", yamlScalar(c.DefaultProvider)); b.WriteString("providers:\n")
	names := []string{"openai", "gemini", "gemini-normalizer", "groq", "anthropic", "llamacpp"}; seen := map[string]bool{}
	for _, name := range names { p, ok := c.Providers[name]; if !ok { continue }; seen[name] = true; fmt.Fprintf(&b, "  %s:\n", name); if p.APIKey != "" { fmt.Fprintf(&b, "    api_key: %s\n", yamlScalar(p.APIKey)) }; if p.DefaultModel != "" { fmt.Fprintf(&b, "    default_model: %s\n", yamlScalar(p.DefaultModel)) }; if p.BaseURL != "" { fmt.Fprintf(&b, "    base_url: %s\n", yamlScalar(p.BaseURL)) } }
	otherNames := make([]string, 0, len(c.Providers)); for name := range c.Providers { if !seen[name] { otherNames = append(otherNames, name) } }; sort.Strings(otherNames)
	for _, name := range otherNames { p := c.Providers[name]; fmt.Fprintf(&b, "  %s:\n", name); if p.APIKey != "" { fmt.Fprintf(&b, "    api_key: %s\n", yamlScalar(p.APIKey)) }; if p.DefaultModel != "" { fmt.Fprintf(&b, "    default_model: %s\n", yamlScalar(p.DefaultModel)) }; if p.BaseURL != "" { fmt.Fprintf(&b, "    base_url: %s\n", yamlScalar(p.BaseURL)) } }
	order := make([]string, len(c.FallbackOrder))
	for i, providerName := range c.FallbackOrder { order[i] = yamlScalar(providerName) }
...
func parseYAML(text string, c *Config) error {
	...
	for _, raw := range strings.Split(strings.ReplaceAll(text, "\r\n", "\n"), "\n") {
		line := strings.TrimRight(raw, " \t"); if strings.TrimSpace(line) == "" || strings.HasPrefix(strings.TrimSpace(line), "#") { continue }; trimmed := strings.TrimSpace(line); indent := len(line) - len(strings.TrimLeft(line, " "))
