	...
	return map[string]any{
		...
		"properties": map[string]any{
			"message": map[string]any{
				"type":        "string",
				"description": "Natural-language response when no file changes are required.",
			},
		...
		"propertyOrdering": []string{"message", "files", "explanation", "commands"},
	}
}
...
func (e *Engine) normalizeChatResponse(ctx context.Context, raw string, parseErr error) (ChatResponse, error) {
	...
	resp, err := e.Registry.Send(ctx, JSONNormalizerProvider, messages, provider.RequestOptions{
		Temperature: 0,
		MaxTokens:   12000,
		JSONMode:    true,
