...
Examples:
  aicli ask "Explain this project"
  aicli ask "Explain this project" --provider gemini
  aicli ask "Create a Go API" --provider llamacpp
  aicli ask "Create a Go API" --provider auto --yes`,
			)
			return nil
	...
	if strings.TrimSpace(prompt) == "" {
		return fmt.Errorf(
			"ask requires a prompt or stdin input",
		)
	}
...
Examples:
  aicli models --provider gemini
  aicli models --provider llamacpp`,
			)
			return nil

		default:
			return fmt.Errorf(
				"unknown models option %q",
				args[i],
			)
		}
	}
	...
	if providerName == "auto" {
		return fmt.Errorf(
			"models requires a specific provider",
		)
	}
	...
	models, err := a.Registry.ListModels(
		context.Background(),
		providerName,
	)
	if err != nil {
...
func configCommand(args []string) error {
	...
	case "set":
		if len(args) != 3 {
			return fmt.Errorf(
				"usage: aicli config set <key> <value>",
			)
		}
	...
	case "show":
		...
		fmt.Println(
			"verification.self_correction_attempts:",
...
Examples:
  aicli config set default_provider llamacpp
  aicli config set fallback_order "[llamacpp, gemini, groq, openai]"
  aicli config set providers.gemini.api_key "YOUR_KEY"`,
		)
		return nil
...
func profileCommand(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf(
			"profile requires show or reset",
		)
	}
